...
let main = async () => {
  ...
  let owner = new ethers.Wallet(
    getOwnerPrivateKey(network.name),
    ethers.provider
  );
  ...
  const instanceConfigAddress = ConfigAddressFactory.connect(owner).attach(
    tmpaddr
  ) as ConfigAddress;
  ...
  let instanceUSDT = (await USDTFactory.connect(owner).deploy(
    "Test BUSDT",
    "BUSDT",
    6
  )) as ERC20;
  console.log(
    ...
    await instanceUSDT["symbol()"]()
  );

  const GameBallotFactory = await ethers.getContractFactory("GameBallot");
  //address GST
  const instanceGameBallot = (await GameBallotFactory.connect(owner).deploy(
  ...
  const instanceGamePair = (await GamePairFactory.connect(
    owner
  ).deploy()) as GamePair;
  ...
  const instanceGameFactory = (await GameFactoryFactory.connect(owner).deploy(
    instanceGSTTOKEN.address
  )) as GameFactory;
  ...
  if (tokens != null) {
    for (let index = 0; index < tokens.length; index++) {
      ...
      console.log(
        "instanceConfigAddress.upsertGameToken:",
        ret.transactionHash
      );
    }
  }
  const ERC20Factory = await ethers.getContractFactory("ERC20");
  let instanceERC20 = (await ERC20Factory.connect(owner).deploy(
    "Test BOST",
    "BOST",
    18
  )) as ERC20;
