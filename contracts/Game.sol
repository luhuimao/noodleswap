...
contract Game is IGame, GameERC20, ConfigurableParametersContract {
    ...
    function isGameClose() private returns (uint256 gameResultType) {
        if (
            ...
            confirmResultTime > 0 && challengeAddress != address(0) && challengeTime + voteSlot < block.timestamp
        ) {
            gameResultType = 2;
        } else if (
            confirmResultTime == 0 &&
            startVoteTime >= endTime + confirmResultSlot &&
            startVoteTime <= endTime + confirmResultSlot + voteSlot &&
            endTime + confirmResultSlot + voteSlot < block.timestamp
        ) {
            gameResultType = 3;
        } else if (
            confirmResultTime == 0 &&
    ...
    function openGame(uint8 _winOption) public override {
        ...
        require(
            endTime < block.timestamp && endTime + confirmResultSlot > block.timestamp,
            'NoodleSwap: not open time'
        );
    ...
    function addVote(uint8 option) public override {
        ...
        if (challengeTime > 0 && challengeTime + voteSlot > block.timestamp) {
            ...
            confirmResultTime == 0 &&
            startVoteTime == 0 &&
            endTime + confirmResultSlot < block.timestamp
        ) {
            canVote = true;
        } else if (
            confirmResultTime == 0 && startVoteTime != 0 && endTime + confirmResultSlot + voteSlot > block.timestamp
        ) {
            canVote = true;
        } else {
            canVote = false;
    ...
    function getConfirmAward() public {
        require(
            msg.sender == confirmResultAddress && originOption == winOption && confirmResultAward == 100,
            'NoodleSwap: Confirmresult address has no award'
        );
    ...
    function getVoteAward() public override {
        ...
        if (voteNumber > 0){
            require(ILockNoodleERC20(lockNoodleToken).balanceOf(address(this)) >= voteNumber,  'Insurfficient Lock Noodle Balance');
            ILockNoodleERC20(lockNoodleToken).transfer(msg.sender, voteNumber);
        }
